...
/**
 * @title SimpleVotingV2
 * @author Sue
 * @notice 基于 Semaphore 协议的匿名投票系统
 ...
 * - 保留原有业务逻辑
 */
contract SimpleVotingV2 is Initializable, UUPSUpgradeable, SemaphoreGroups {
    // ========== 数据结构 ==========
    ...
    struct Proposal {
        ...
        mapping(uint256 => bool) usedNullifiers; // nullifier => 是否已使用
     ...
     * @param nullifier 无效化器 (防重复投票)
     * @param proof ZK 证明 (Groth16)
     *
     * @dev 工作流程:
     * 1. 验证业务逻辑 (提案存在、未过期、选项合法、nullifier 未使用)
     * 2. 验证 ZK 证明 (调用 Semaphore 验证器)
     * 3. 记录投票 (增加票数、标记 nullifier 已使用)
     *
     * @dev ZK 证明保证:
     * - 投票者是群组成员
    ...
    function vote(
        ...
        require(!p.usedNullifiers[nullifier], "Already voted");
        ...
        p.usedNullifiers[nullifier] = true;
     ...
     * @return 是否已使用
     */
    function isNullifierUsed(uint256 proposalId, uint256 nullifier)
        external
        view
        returns (bool)
    {
        return proposals[proposalId].usedNullifiers[nullifier];
    }
